...
# BGP peer East NVA to Vwan
echo -e "$WHITE$(date +"%T")$GREEN Create BGP peering to NVA (East) $WHITE"
id=$(az network vhub connection show -n nvaeasttohub -g $rg --vhub-name hubeast --query 'id' -o tsv)
az network vhub bgpconnection create -n frrEastPeer1 -g $rg --vhub-name hubeast --peer-asn 65001 --peer-ip "10.1.0.10" --vhub-conn $id -o none

# BGP peer West NVA to Vwan
echo -e "$WHITE$(date +"%T")$GREEN Create BGP peering to NVA (West) $WHITE"
id=$(az network vhub connection show -n nvawesttohub -g $rg --vhub-name hubwest --query 'id' -o tsv)
az network vhub bgpconnection create -n frrWeastPeer1 -g $rg --vhub-name hubwest --peer-asn 65002 --peer-ip "10.2.0.10" --vhub-conn $id -o none
